...
by_ids_array = """
Эндпоинт преднозначен для уменьшения количество запросов на получения услугу.
Получение услуг по массиву id;
**Параметры запроса:**
```
- `ids`: Объяззательный массив в котором должны быть id услуг в String.
```
...
**Ответ:**
```
Если массив был отправлен и он не пустой, то возвращается массив с услугами.
```
**Ответы:**
```
-` 200 OK: Успешный запрос, услуги успешно получены.
-` 422: Массив не отправлен или же он пуст.
...
"""
get_booked_services = """
Данный эндпоинт возвращает список с забронированными услугами авторизованного пользователя и пользователями которые забронировали их.
...
"""
get_my_services = """
Данный эндпоинт преднозначен для получения услуг атворизованного пользователя.

**Параметры:**
```
- `'Authorization': `Bearer ${idToken}`
- `'uid': ID пользователя, для проверки на правильный токен
```
**Пример ответа:**
```
data = [
...
- `200 OK`: Успешный запрос, данные успешно получены.
- `403 Forbidden`: Токен доступа отсутствует или недействителен, или у пользователя недостаточно прав.
- `404 Not` Found: Усллуг не найдено.
"""

like_service_event = """
Эндпоинт преднозначен для добавления услуги в лайки пользователя.
Если усулги не было в лайках, то он добавляется, если же был то удаляется из массива.
Массив лайков автоматически удаляется, если в нём нету элементов.

**Параметры:**
```
- `'Authorization': `Bearer ${idToken}`
- `'uid': ID пользователя, для проверки на правильный токен
- `'service_id': id услуги с которым надо производить действия
```
**Ответы:**
```
- `200 OK`: Улсуга успешна удалена из массива лайков пользователя.
- `201 OK`: Услуга успешно добавлена в массив лайков.
- `403 Forbidden`: Токен доступа отсутствует или недействителен, или у пользователя недостаточно прав.
- `404 Not` Found: Усллуг не найдено.
- `500 Internal Error` : Проблемы на стороне сервера.
"""

add_new_review = """
Эндпоинт добавляет новый комментайри к усулге, если:
    Пользователь авторизован,
    Указан id услуги,
    Указано количество звёзд,
    Пользователь ещё не комментировал данную услугу

**Параметры:**
```
- `'Authorization': `Bearer ${idToken}`
- `'uid': ID пользователя, для проверки на правильный токен, и комментировал ли уже плоьзователь
- `'service_id': id услуги с которым надо производить действия
- `'rating'`: Количетсво звёзд
- `'message'`: Сообщение (Необъязательно)
```
**Ответы:**
```
- `200 OK`: Комментарий успешно добавлен.
- `403`: Пользователь не аутентифицирован.
- `404`: Пользоватеьль или учлуга не найден.
- `405`: Пользователь уже комментировал данную услугу.
- `422`: Неправильно указанные данные.
- `500`: Ошибка на стороне сервера.
"""

get_service_reviews = """
Эндпоинт возваращает массив с объекстами комментариве усулги, исходя из указанного id услуги.
...
**Ответы:**
```
- `200 OK`: Ответ с массивом комментариев услуги с аватаркой и именем пользователя.
- `422`: service_id не указан.
- `500`: Ошибка на стороне сервера.
"""

change_booking_status = """
Данный эндпоинт изменяет статус забронированной услуги, если изменить пытается владелец услуги.
...
- `'uid': ID пользователя, для проверки на правильный токен, и комментировал ли уже плоьзователь
- `'booking_id': id брони
- `'status'`: Новый статус - Active, Complited, Cancaled
```
**Ответы:**
```
- `200 OK`: Ответ с массивом комментариев услуги с аватаркой и именем пользователя.
- `403`: Пользователь не авторизован или не является владельцем.
- `404`: пользователя или забронированной услуги не найдено.
- `422`: Отправленные данные не совпадают с ожиданиаем сервера.
- `500`: Ошибка на стороне сервера.
"""